...
# plot data and f(x)
plot(interploated_data$midpoint, interploated_data$fairway )
plot(interploated_data$midpoint, interploated_data$rough )
ggplot(data = interploated_data, aes(x = yards)) +
  scale_y_continuous(
...
new.model <- loess(fairway ~ yards, data = interploated_data, span = 0.25)
summary(new.model)
plot(new.model,main = "Rough Expected Strokes Green", xlab = "Approach Distance to Green (yards)", ylab = "Expected Stroked Remaining")


new.model <- loess(rough ~ yards, data = interploated_data, span = 0.25)
summary(new.model)
